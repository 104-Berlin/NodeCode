...
class Node {
 public:
  ...
  virtual std::string getCCode(std::vector<Connection*> inputs,
                               std::vector<Connection> outputs) = 0;
};
...
class CNode : public Node {
 ...
 public:
  CNode(std::vector<NodeInOut> in, std::vector<NodeInOut> out,
        std::vector<std::string> cParts, std::vector<std::string> filler);
  std::string getCCode(std::vector<Connection*> inputs,
...
class FunctionNode : public Node {
 ...
 public:
  ...
  FunctionNode(std::vector<NodeInOut> in, std::vector<NodeInstance*> innerNodes,
               std::vector<NodeInOut> out);
  std::string getCCode(std::vector<Connection*> inputs,
                       std::vector<Connection> outputs);
  std::string getFunctionCode();
  std::string getName();
  bool connectWith(std::string funcOut, NodeInstance* node,
                   std::string nodeOut);
};
...
class NodeInstance {
 ...
 public:
  ...
  bool connectWith(std::string inName, FunctionNode* funcNode,
                   std::string funcIn);
  bool connectWith(std::string inName, NodeInstance* prevNode,
                   std::string outName);
...
std::string NC_getCProg(std::vector<NodeInstance*> main,
                        std::vector<FunctionNode*> funcs,
                        std::vector<CNode*> cNodes);
void NC_compileAndRun(std::vector<NodeInstance*> main,
                      std::vector<FunctionNode*> funcs,
                      std::vector<CNode*> cNodes);
}  // namespace NodeCode
